"""HTTP-level tests for /api/sentinel-analytics-rules/* -- mirrors
test_sentinel_hunt_endpoints.py's structure and fixture. The underlying
modules (sentinel_analytics_rules_sync.py, sentinel_analytics_rules.py)
have their own dedicated unit tests; this focuses on auth gating, 404/400
handling, and request/response wiring."""
...
def _seed_rule(conn, review_state="pending"):
    row = conn.execute(
        "INSERT INTO sentinel_analytics_rules (sentinel_rule_id, display_name, kql_body, review_state) "
        "VALUES (?, ?, ?, ?) RETURNING id",
        ("rule-1", "Rule 1", "T | take 1", review_state),
    ).fetchone()
    conn.commit()
    return row["id"]
...
@pytest.mark.parametrize("method,path", [
    ("get", "/api/sentinel-analytics-rules"),
    ("get", "/api/sentinel-analytics-rules/1"),
])
...
@pytest.mark.parametrize("method,path", [
    ("post", "/api/sentinel-analytics-rules/sync"),
    ("post", "/api/sentinel-analytics-rules/1/test"),
    ("post", "/api/sentinel-analytics-rules/1/tune"),
    ("patch", "/api/sentinel-analytics-rules/1/review"),
    ("post", "/api/sentinel-analytics-rules/1/tuning-suggestion/apply"),
    ("post", "/api/sentinel-analytics-rules/1/tuning-suggestion/dismiss"),
])
...
def test_list_empty(api_client):
    ...
    response = client.get(
        "/api/sentinel-analytics-rules", headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    assert response.json() == {"total": 0, "items": []}


def test_list_caps_limit_at_200(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.get(
        "/api/sentinel-analytics-rules?limit=9999",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200


def test_list_rejects_invalid_disposition(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.get(
        "/api/sentinel-analytics-rules?disposition=bogus",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 400


def test_list_filters_by_disposition_and_search(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    conn = db_module._connect_db()
    conn.execute(
        "INSERT INTO sentinel_analytics_rules "
        "(sentinel_rule_id, display_name, kql_body, backtest_disposition) "
        "VALUES (?, ?, ?, ?)",
        ("rule-tune", "Suspicious PowerShell Download", "T | take 1", "needs_tuning"),
    )
    conn.execute(
        "INSERT INTO sentinel_analytics_rules "
        "(sentinel_rule_id, display_name, kql_body, backtest_disposition) "
        "VALUES (?, ?, ?, ?)",
        ("rule-clean", "Scheduled Task Creation", "T | take 1", "clean"),
    )
    conn.commit()

    response = client.get(
        "/api/sentinel-analytics-rules?disposition=needs_tuning",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["total"] == 1
    assert body["items"][0]["sentinel_rule_id"] == "rule-tune"

    response = client.get(
        "/api/sentinel-analytics-rules?search=powershell",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    body = response.json()
...
def test_get_rule_404(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.get(
        "/api/sentinel-analytics-rules/999999",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 404


def test_get_rule_found_includes_kql_body(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    conn = db_module._connect_db()
    rule_id = _seed_rule(conn)

    response = client.get(
        f"/api/sentinel-analytics-rules/{rule_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
...
def test_sync_disabled_returns_noop(api_client, monkeypatch):
    ...
    response = client.post(
        "/api/sentinel-analytics-rules/sync", headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
...
def test_test_endpoint_404_for_missing_rule(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.post(
        "/api/sentinel-analytics-rules/999999/test", headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 404


def test_test_endpoint_no_sentinel_client_records_error(api_client):
    ...
    response = client.post(
        f"/api/sentinel-analytics-rules/{rule_id}/test",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    body = response.json()
...
def test_tune_endpoint_404_for_missing_rule(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.post(
        "/api/sentinel-analytics-rules/999999/tune", headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 404
...
def test_tune_endpoint_requires_needs_tuning_disposition(api_client):
    ...
    conn = db_module._connect_db()
    rule_id = _seed_rule(conn)
    sentinel_analytics_rules.record_rule_test_result(conn, rule_id, {"tables": []}, "clean")

    response = client.post(
        f"/api/sentinel-analytics-rules/{rule_id}/tune",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 400
...
def test_review_endpoint_updates_state(api_client):
    ...
    response = client.patch(
        f"/api/sentinel-analytics-rules/{rule_id}/review",
        headers={"Authorization": f"Bearer {token}"},
...
def test_review_endpoint_404_for_missing_rule(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.patch(
        "/api/sentinel-analytics-rules/999999/review",
        headers={"Authorization": f"Bearer {token}"},
        json={"review_state": "approved"},
    )
    assert response.status_code == 404
...
def test_dismiss_tuning_suggestion_endpoint_404_for_missing_rule(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.post(
        "/api/sentinel-analytics-rules/999999/tuning-suggestion/dismiss",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 404
...
def test_dismiss_tuning_suggestion_endpoint_success(api_client):
    ...
    conn = db_module._connect_db()
    rule_id = _seed_rule(conn)
    sentinel_analytics_rules.record_rule_tune_result(
        conn, rule_id, {"disposition": "tuned", "final_body": "T | take 1 | where X != 1"},
    )

    response = client.post(
        f"/api/sentinel-analytics-rules/{rule_id}/tuning-suggestion/dismiss",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    body = response.json()
    assert body["result"]["success"] is True
    assert body["rule"]["tuning_suggestion"]["action"] == "dismissed"


def test_apply_tuning_suggestion_endpoint_404_for_missing_rule(api_client):
    client, main_mod = api_client
    token = _admin_token(main_mod)
    response = client.post(
        "/api/sentinel-analytics-rules/999999/tuning-suggestion/apply",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 404
...
def test_apply_tuning_suggestion_endpoint_reports_failure_when_sync_disabled(api_client, monkeypatch):
    ...
    response = client.post(
        f"/api/sentinel-analytics-rules/{rule_id}/tuning-suggestion/apply",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 200
    body = response.json()
